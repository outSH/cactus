...
describe("InvokeWeb3EthMethodEndpoint Tests", () => {
  ...
  test("invokeWeb3EthMethod with 0-argument method works (getGasPrice)", async () => {
  ...
  test("invokeWeb3EthMethod with missing arg throws error (getBlock)", async () => {
    try {
      const connectorResponse = connector.invokeWeb3EthMethod({
        methodName: "getBlock",
      });
  ...
  test("invokeWeb3EthMethod with invalid arg throws error (getBlock)", async () => {
    try {
      const connectorResponse = connector.invokeWeb3EthMethod({
        methodName: "getBlock",
        params: ["foo"],
  ...
  test("invokeWeb3EthMethod with non existing method throws error", async () => {
    try {
      const connectorResponse = connector.invokeWeb3EthMethod({
        methodName: "foo",
        params: ["foo"],
