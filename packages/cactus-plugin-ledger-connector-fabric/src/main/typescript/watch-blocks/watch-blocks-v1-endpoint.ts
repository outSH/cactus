 ...
 */
export function safeStringifyException(error: unknown) {
...
export class WatchBlocksV1Endpoint {
  ...
  private getBlockListener(type: WatchBlocksListenerTypeV1) {
    ...
    if (!listener || !listenerType) {
      // Should never happen
      throw new Error("Could not determine listener or listenerType.");
    }
   ...
   */
  public async subscribe(options: WatchBlocksOptionsV1) {
    const { socket, log } = this;
    const clientId = socket.id;
