...
import Client, { Proposal, ProposalRequest, ProposalResponse } from "fabric-client";
import { FileSystemWallet, Gateway } from "fabric-network";

import { getClientAndChannel, getSubmitterAndEnroll } from "./fabricaccess";
import { signProposal } from "./sign-utils";
import { ProposalSerializer, ProposalResponseSerializer } from "./fabric-proto-serializers"
...
export class ServerPlugin {
  ...
  async sendSignedTransactionV2(args: SendSignedTransactionV2Args) {
    ...
    let proposalResponses: any[] = args.args.args.proposalResponses.map((val) => ProposalResponseSerializer.decode(val));
    let reqID = args.reqID;
    logger.info(`##sendSignedTransactionV2: reqID: ${reqID}`);

    if (!channelName || !signedCommitProposal || !proposal || proposalResponses.length === 0) {
      throw {
        resObj: {
          status: 504,
  ...
  async sendSignedProposal(args: SendSignedProposalArgs) {
    ...
    try {
      ...
      const { proposal, txId } = InvokeGenerateUnsignedProposal(channel, transactionProposalReq, certPem);
      const signedProposal = signProposal(proposal.toBuffer(), privateKeyPem);

      // Send proposal, get endorsment responses
      const { endorsmentStatus, proposalResponses } = await InvokeSendSignedProposalV2(
        channel,
      ...
      const commitProposal = await InvokeGenerateUnsignedTransaction(channel, proposalResponses as any, proposal);
  ...
  async sendSignedProposalV2(args: SendSignedProposalV2Args) {
    ...
    try {
      ...
      let proposalResponses = invokeResponse.proposalResponses.map((val: any) => ProposalResponseSerializer.encode(val));
      logger.debug(`sendSignedProposalV2: encoded ${proposalResponses.length} proposalResponses.`);
   ...
   * @returns txId Transaction ID object.
   */
  async generateUnsignedProposal(args: GenerateUnsignedProposalArgs) {
    ...
    try {
      ...
      let invokeResponse = InvokeGenerateUnsignedProposal(channel, transactionProposalReq, certPem);
      if (!invokeResponse.proposal || !invokeResponse.txId) {
        throw new Error("generateUnsignedProposal: empty proposal or transaction id.");
      }
  ...
  async generateUnsignedTransaction(args: GenerateUnsignedTransactionArgs) {
    ...
    const proposal = ProposalSerializer.decode(args.args.args.proposal)
    let proposalResponses: any[] = args.args.args.proposalResponses.map((val) => ProposalResponseSerializer.decode(val));
    logger.debug(`##generateUnsignedTransaction Received ${proposalResponses.length} proposal responses`);
    let reqID = args.reqID;
    ...
    try {
      ...
      const txProposal = await InvokeGenerateUnsignedTransaction(channel, proposalResponses, proposal);
 ...
 */
function InvokeGenerateUnsignedProposal(channel: Client.Channel, txProposal: ProposalRequest, certPem: string) {
  if (!txProposal || !certPem) {
    throw new Error("InvokeGenerateUnsignedProposal: Invalid input parameters.");
  }
 ...
 */
async function InvokeGenerateUnsignedTransaction(channel: Client.Channel, proposalResponses: ProposalResponse[], proposal: Proposal) {
  if (!proposal || !proposalResponses || proposalResponses.length === 0) {
    throw new Error("InvokeGenerateUnsignedTransaction: Invalid input parameters.");
  }
...
async function InvokeSendSignedProposalV2(
  ...
  logger.debug("##InvokeSendSignedProposalV2: successfully sent signedProposal");
