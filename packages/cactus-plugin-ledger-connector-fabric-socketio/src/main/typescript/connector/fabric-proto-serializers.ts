...
// This connector package.json is outside of rootDir, ignore this error.
// @ts-ignore
import { version as fabricConnectorVersion, dependencies as fabricConnectorDependencies } from '../../../../package.json';
const fabricSdkVersion = fabricConnectorDependencies["fabric-client"];
...
function decodeMetdata(typeName: string, encodedMessage: string): string {
  ...
  if (metadata.typeName !== typeName) {
    throw new Error(`decodeMetdata(): requested type mismatch. Wanted to decode: ${typeName}, received: ${metadata.typeName}`);
  }
  if (metadata.fabricSdkVersion != fabricSdkVersion) {
    throw new Error(`decodeMetdata(): fabric SDK mismatch. Local version: ${typeName}, message encoded with: ${metadata.typeName}`);
  }
  if (metadata.fabricConnectorVersion != fabricConnectorVersion) {
    throw new Error(`decodeMetdata(): connector version mismatch. Local version: ${fabricConnectorVersion}, message encoded with: ${metadata.fabricConnectorVersion}`);
  }
...
// Protobuf builders
const proposalBuilder = loadFabricProto("fabric-client/lib/protos/peer/proposal.proto");
const proposalResponseBuilder = loadFabricProto("fabric-client/lib/protos/peer/proposal_response.proto");
...
export namespace ProposalSerializer {
  ...
  export function decode(encodedProposal: string): Client.Proposal {
    return ProposalType.decodeJSON(decodeMetdata(proposalTypeName, encodedProposal));
  }
}
...
export namespace ProposalResponseSerializer {
  export const ProposalResponseType = proposalResponseBuilder.protos.ProposalResponse;
  ...
  export function encode(proposalResponse: Client.ProposalResponse): string {
    let proposalResponseCopy = cloneDeep(proposalResponse) as Record<string, any>;

    // Peer is not part of protobuf definition, remove it.
    delete proposalResponseCopy.peer;

    let proposalResponseMessage = new ProposalResponseType(proposalResponseCopy);
  ...
  export function decode(encodedProposalResponse: string) {
    let decodedProposalResponse = ProposalResponseType.decodeJSON(decodeMetdata(proposalResponseTypeName, encodedProposalResponse));
