...
describe("Fabric-SocketIO connector tests", () => {
    ...
    if (leaveLedgerRunning || useRunningLedger) {
      ...
      try {
        ...
        if (!err.message.includes("EEXIST")) {
          log.error("Unexpected exception when creating common wallet dir:", err);
          throw err;
        }
      }
    } else {
   ...
   */
  async function getUnsignedCommitProposal(txProposal: unknown, proposalResponses: unknown) {
    const contract = { channelName: ledgerChannelName };
  ...
  test("Signing proposal creates a signature", async () => {
    ...
    const signedProposal = connectorModule.signProposal(proposal, privateKeyPem);
  ...
  test("Sending transaction signed on BLP side (without sharing the private key) works", async () => {
    ...
    const { proposal, proposalBuffer } = await getUnsignedProposal(txProposal, certPem);

    // Prepare signed proposal
    const signedProposal = connectorModule.signProposal(proposalBuffer, privateKeyPem);
    ...
    const responseSignedProposal = await apiClient.sendSyncRequest(contractSignedProposal, methodSignedProposal, argsSignedProposal);
    ...
    const commitProposalBuffer = await getUnsignedCommitProposal(proposal, proposalResponses);

    // Prepare signed commit proposal
    const signedCommitProposal = connectorModule.signProposal(commitProposalBuffer, privateKeyPem);

    // Call sendSignedTransactionV2
    const contractSignedTransaction = { channelName: ledgerChannelName };
    const methodSignedTransaction = { type: "function", command: "sendSignedTransactionV2" };
    ...
    const responseSignedTransaction = await apiClient.sendSyncRequest(contractSignedTransaction, methodSignedTransaction, argsSignedTransaction);
